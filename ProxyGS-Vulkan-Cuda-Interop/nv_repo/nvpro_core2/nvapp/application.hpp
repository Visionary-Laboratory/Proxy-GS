...
public:
  ...
  // Headless frame-by-frame control
  bool headlessInit();     // Initialize headless mode (call once)
  bool headlessStep();     // Render one frame, returns false when done
  void headlessShutdown(); // Cleanup headless mode
...
private:
  ...
  bool                  m_headlessInitialized{false};
  uint32_t              m_headlessFrameCur{0};
