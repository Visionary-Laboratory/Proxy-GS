...
//-----------------------------------------------------------------------
// Initialize headless mode (three-part system)
// This should be called once before any headlessStep() calls
//
bool nvapp::Application::headlessInit()
{
  if (m_headlessInitialized) return true;
  ...
  // Need to render the UI once to set up the internal state and layout
  ImGui_ImplVulkan_NewFrame();
  ImGui::NewFrame();
  setupImguiDock();

  // Call UI rendering for each element (setup pass)
  for(std::shared_ptr<IAppElement>& e : m_elements)
  {
    e->onUIRender();
  }
  ...
  static constexpr int32_t kNumFramesInFlight = 1;
  createFrameSubmission(kNumFramesInFlight);

  m_headlessInitialized = true;
  m_headlessFrameCur = 0;
...
// Returns false when all frames are done or close is requested
//
bool nvapp::Application::headlessStep()
{
  if (!m_headlessInitialized) headlessInit();
  if (m_headlessFrameCur >= m_headlessFrameCount || m_headlessClose) {
    return false; // Tell caller: no more frames
  }
  ...
  static constexpr int32_t kNumFramesInFlight = 1;
                // Wait for previous frame fence (<= numFramesInFlight)

  prepareFrameToSignal(kNumFramesInFlight);

  VkCommandBuffer cmd = beginCommandRecording(); // Start the command buffer
  // printf("begin fraw frame");
  drawFrame(cmd);                                // Call onUIRender() and onRender() for each element
  endFrame(cmd, kNumFramesInFlight);             // End the frame and submit it
  advanceFrame(kNumFramesInFlight);              // Advance to the next frame in the ring buffer
  ...
  return (m_headlessFrameCur < m_headlessFrameCount) && !m_headlessClose;
}

//-----------------------------------------------------------------------
// Shutdown headless mode (three-part system)
// This should be called when headlessStep() returns false
//
void nvapp::Application::headlessShutdown()
{
  if (!m_headlessInitialized) return;

  ImGui::Render(); // Generate UI draw data (CPU)
  vkDeviceWaitIdle(m_device); // Ensure GPU finishes
...
void nvapp::Application::headlessRun()
{
  ...
  headlessInit();
  printf("finish headlessInit\n");
  // Render frames one by one until done
  while (headlessStep()) {
    printf("step");
    // headlessStep() returns false when all frames are done
  }
